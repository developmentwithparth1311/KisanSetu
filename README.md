...
default code block with JavaScript example:
documentation for real-time streaming ASR.
javascript
// Real-time streaming ASR
const sarvamASR = await fetch('https://api.sarvam.ai/speech-to-text-translate', {
  method: 'POST',
  headers: {
'type': 'application/json', 'API-Subscription-Key': process.env.SARVAM_API_KEY,
'type': 'application/json'
dbody: JSON.stringify({'
language_code': 'hi-IN', // Hindi model: 'saaras:v1', audio: audioStreamBase64, enable_punctuation: true })});
documentation for conversational AI.
javascript
// Intent extraction and response generation
const sarvamLLM = await fetch('https://api.sarvam.ai/chat', {
  method: 'POST',
  headers: {
'type': 'application/json', 'API-Subscription-Key': process.env.SARVAM_API_KEY,
'type': 'application/json'
dbody: JSON.stringify({'
messages': [ { role: 'system', content: 'You are KisanSetu voice assistant helping farmers with crop prices and sales. Extract intent and entities from farmer queries.' }, { role: 'user', content: transcribedText // from Saaras } ], '
language': 'hi', // Hindi temperature: 0.3 // lower for more consistent entity extraction })});
documentation for Text-to-Speech.
javascript
// Convert response to natural speech
const sarvamTTS = await fetch('https://api.sarvam.ai/text-to-speech', {
  method: 'POST',
  headers: {
'type': 'application/json', 'API-Subscription-Key': process.env.SARVAM_API_KEY,
'type': 'application/json'
dbody: JSON.stringify({'
to convert response to natural speech.
the text of the response, language code, speaker, pitch, pace, loudness etc.'})});}
takes the generated audio stream to pipe back through Twilio.
default cost estimation details:
asr_saras ~₹0.40 per minute of audio,
lm_m ~₹0.002 per 1K tokens,
tts_bulbul ~₹0.25 per 1K characters.
typical call costs about ₹1.12 per interaction.
budget info:
the free tier includes 5000 API calls/month suitable for hackathon demo + initial pilot.
