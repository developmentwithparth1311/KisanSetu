...
export async function POST(request: Request) {
  try {
    ...
    // 1. Entity Extraction - Crop Detection (English & Hindi/Hinglish)
    let detectedCropId = 'tomato';
    if (transcript.includes('onion') || transcript.includes('pyaz') || transcript.includes('pyaaz') || transcript.includes('प्याज़') || transcript.includes('कांदा')) {
      detectedCropId = 'onion';
    } else if (transcript.includes('potato') || transcript.includes('aloo') || transcript.includes('aalu') || transcript.includes('आलू') || transcript.includes('बटाटा')) {
      detectedCropId = 'potato';
    } else if (transcript.includes('wheat') || transcript.includes('gehu') || transcript.includes('gehoon') || transcript.includes('गेहूं') || transcript.includes('गहू')) {
      detectedCropId = 'wheat';
    } else if (transcript.includes('soybean') || transcript.includes('soya') || transcript.includes('सोयाबीन')) {
      detectedCropId = 'soybean';
    } else if (transcript.includes('tomato') || transcript.includes('tamatar') || transcript.includes('टमाटर')) {
      detectedCropId = 'tomato';
    }

    // 2. Entity Extraction - Mandi Detection
    ...
    } else if (transcript.includes('delhi') || transcript.includes('azadpur') || transcript.includes('आजादपुर')) {
      detectedMandiId = 'azadpur';
    } else if (transcript.includes('nashik') || transcript.includes('नाशिक') || transcript.includes('नासिक')) {
    ...
    const trendText = advisory.pctTrend7Day >= 0 ? `up ${advisory.pctTrend7Day}% this week` : `down ${Math.abs(advisory.pctTrend7Day)}% this week`;

    // 4. Generate Natural Spoken Dialogue (English & Hindi)
